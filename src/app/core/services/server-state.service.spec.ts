...
describe('ServerStateService', () => {
  ...
  it('should map server states correctly', () => {
    ...
    expect(service.mapServerState('error')).toBe('错误');
    expect(service.mapServerState('already-running')).toBe('已在运行');
    expect(service.mapServerState('instance-folder-missing')).toBe('实例文件夹丢失');
