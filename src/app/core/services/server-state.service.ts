...
export class ServerStateService {
  ...
  mapServerState(state: string | null | undefined): string {
    if (!state || state.toLowerCase() === 'unknown') return '未知';
    switch (state.toLowerCase()) {
      case 'queued': return '准备启动';
      case 'starting': return '启动中';
      case 'running': return '运行中';
      case 'stopping': return '停止中';
      case 'stopped': return '已停止';
      case 'error': return '错误';
      case 'already-running': return '已在运行';
      case 'instance-folder-missing': return '实例文件夹丢失';
  ...
  areSettingsLocked(state: string | null | undefined): boolean {
    const mappedState = this.mapServerState(state);
    return mappedState === '准备启动' || mappedState === '启动中' || mappedState === '停止中' || mappedState === '运行中';
  }
  ...
  areBackupSettingsLocked(state: string | null | undefined): boolean {
    ...
    return mappedState === '启动中' || mappedState === '停止中';
  }

  /**
   * Send an RCON command to a specific instance
   */
  sendRconCommand(instanceId: string, command: string): void {
    if (!instanceId || !command) return;
    this.messaging.sendMessage('rcon-send-command', { instanceId, command });
  }
