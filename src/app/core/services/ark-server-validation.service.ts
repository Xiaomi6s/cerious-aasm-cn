...
export class ArkServerValidationService {
  ...
  validatePorts(server: any): FieldValidation {
    ...
    for (const port of ports) {
      ...
      // QueryPort is optional — skip validation when 0, null, or undefined
      if (!port.required && (portValue === 0 || portValue === null || portValue === undefined)) {
        continue;
      }
    ...
    // Check for port conflicts (only among ports that are actually set)
    const usedPorts = new Set();
    for (const port of ports) {
      const portValue = port.value !== undefined ? port.value : port.default;
      if (!portValue) continue; // skip 0/null/undefined
