...
export class RconControlComponent {
  ...
  get filteredCommands(): string[] {
    const q = this.rconMessage.trim().toLowerCase();
    if (!q) return this.knownCommands;
    return this.knownCommands.filter(cmd => cmd.toLowerCase().includes(q));
  }
