...
export class ModsTabComponent {
  ...
  onAddMod(): void {
    ...
    if (!id || !name) return;
    if (!/^\d+$/.test(id)) {
      this.notification.error('模组 ID 必须是数字 CurseForge ID（例如 731604991）。请从 CurseForge 网站复制。');
      return;
    }
  ...
  addModFromCurseForge(mod: any): void {
    ...
    if (existing) {
      this.notification.warning(`Mod "${mod.name}" is already in the list.`, '模组浏览器');
      return;
    }
    this.addMod.emit({ id: String(mod.id), name: mod.name });
    this.notification.success(`Added "${mod.name}" (${mod.id}).`, '模组浏览器');
    this.cdr.markForCheck();
  }
