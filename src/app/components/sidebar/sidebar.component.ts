...
export class SidebarComponent implements OnInit, OnDestroy {
      ...
      result.pipe(take(1)).subscribe({
        next: (response) => {
          if (response.success && response.instance) {
            ...
            this.notificationService.success(response.message || '服务器导入成功');
            this.closeAdd();
            // Force change detection to ensure modal closes
            this.cdr.detectChanges();
          } else {
            this.notificationService.error(response.error || '从备份导入服务器失败');
          }
        },
        error: (error) => {
          console.error('Failed to import backup:', error);
          this.notificationService.error('从备份导入服务器失败');
        }
      });
  ...
  onConfirmDelete() {
    if (this.serverToDelete && this.servers.length > 1) {
      ...
      if ((this.serverToDelete.state || '').toLowerCase() !== 'stopped') {
        this.notificationService.error('无法删除服务器', '必须在停止服务器后才能删除。');
        this.onCancelDelete();
        return;
      }
  ...
  onDeleteServer(server: ServerInstance, event: Event) {
    ...
    if ((server.state || '').toLowerCase() !== 'stopped') {
      this.notificationService.warning('无法删除服务器', '必须在停止服务器后才能删除。');
      return;
    }
    ...
    if (this.isWebMode) {
      try {
        ...
        if (response.ok) {
          ...
          this.notificationService.error('登出失败', '身份验证');
        }
      } catch (error) {
  ...
  onConfirmStartAll() {
    ...
    this.serverInstanceService.messaging.sendMessage('start-all-instances', {}).subscribe({
      next: (res: any) => {
        if (res?.success) {
          this.notificationService.success('所有服务器正在启动。', '服务器控制');
        } else {
          this.notificationService.error(res?.error || '启动所有服务器失败。', '服务器控制');
        }
        this.cdr.markForCheck();
      },
      error: () => this.notificationService.error('启动所有服务器失败。', '服务器控制')
    });
  }
  ...
  onConfirmStopAll() {
    ...
    this.serverInstanceService.messaging.sendMessage('stop-all-instances', {}).subscribe({
      next: (res: any) => {
        if (res?.success) {
          this.notificationService.success('所有服务器正在停止。', '服务器控制');
        } else {
          this.notificationService.error(res?.error || '停止所有服务器失败。', '服务器控制');
        }
        this.cdr.markForCheck();
      },
      error: () => this.notificationService.error('停止所有服务器失败。', '服务器控制')
    });
  }
}