...
export class ServerComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  saveSettings() {
    if (this.activeServerInstance && this.activeServerInstance.id) {
      ...
      if (!validation.isValid) {
        ...
        this.notificationService.error('配置验证失败', errorMessage);
        return;
      }
      ...
      if (validation.warnings.length > 0) {
        ...
        this.notificationService.warning('配置警告', warningMessage);
      }
      ...
      if (saveObservable) {
        saveObservable.subscribe((result) => {
          if (result && result.success !== false) {
            ...
            this.notificationService.error('保存失败', '保存服务器配置失败。');
          }
        });
      }
    }
  ...
  onAddMod(modData: {id: string, name: string}) {
    if (!modData.id || !modData.id.trim() || !modData.name || !modData.name.trim()) {
      this.notificationService.error('请输入有效的模组 ID 和名称');
      return;
    }

    if (!/^\d+$/.test(modData.id.trim())) {
      this.notificationService.error('模组 ID 必须是数字 CurseForge ID（例如 731604991）。请从 CurseForge 网站复制。');
      return;
    }
    ...
    if (existingMod) {
      this.notificationService.warning('模组已在列表中');
      return;
    }
    ...
    // Save settings
    this.saveSettings();
    this.notificationService.success('模组添加成功');
  }
  ...
  onRemoveMod(mod: any) {
    ...
    this.saveSettings();
    this.notificationService.success('模组移除成功');
  }
  ...
  onUpdateModSettings(data: {mod: any, settings: any}) {
    ...
    // Save settings
    this.saveSettings();
    this.notificationService.success('模组设置更新成功');
  }
  ...
  saveWorld() {
    if (this.activeServerInstance && this.activeServerInstance.id) {
      this.serverStateService.sendRconCommand(this.activeServerInstance.id, 'SaveWorld');
      this.notificationService.success('存档命令已发送', '服务器正在保存世界数据');
    }
  }

  saveAndBackup() {
    if (!this.activeServerInstance?.id) return;
    const instanceId = this.activeServerInstance.id;
    // 先发 SaveWorld
    this.serverStateService.sendRconCommand(instanceId, 'SaveWorld');
    this.notificationService.info('正在存档，3秒后自动创建备份...', '存档并备份');
    setTimeout(() => {
      // 直接创建备份，不弹窗
      this.backupUIService.createManualBackup(instanceId, this.cdr);
    }, 3000);
  }
  ...
  saveAutoStartSettings(): void {
    ...
    this.automationService.configureAutoStart(serverId, {
      ...
      next: (response) => {
        if (response.success) {
          this.notificationService.success('自动启动设置已保存', '自动化');
        } else {
          this.notificationService.error('保存自动启动设置失败：' + response.error, '自动化');
        }
      },
      error: (error) => {
        this.notificationService.error('配置自动启动失败', '自动化');
  ...
  saveCrashDetectionSettings(): void {
    ...
    this.automationService.configureCrashDetection(serverId, {
      ...
      next: (response) => {
        if (response.success) {
          this.notificationService.success('崩溃检测设置已保存', '自动化');
        } else {
          this.notificationService.error('保存崩溃检测设置失败：' + response.error, '自动化');
        }
      },
      error: (error) => {
        this.notificationService.error('配置崩溃检测失败', '自动化');
  ...
  saveScheduledRestartSettings(): void {
    ...
    this.automationService.configureScheduledRestart(serverId, {
      ...
      next: (response) => {
        if (response.success) {
          this.notificationService.success('计划重启设置已保存', '自动化');
        } else {
          this.notificationService.error('保存计划重启设置失败：' + response.error, '自动化');
        }
      },
      error: (error) => {
        this.notificationService.error('配置计划重启失败', '自动化');
