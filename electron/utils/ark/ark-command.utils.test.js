...
const ark_command_utils_1 = require("./ark-command.utils");
...
describe('ArkCommandUtils', () => {
    describe('prepareArkServerCommand', () => {
        ...
        it('should prepare command for Windows', () => {
            ...
            const result = ark_command_utils_1.ArkCommandUtils.prepareArkServerCommand('C:/ark/ark.exe', ['-foo', '-bar'], { TEST: '1' });
        ...
        it('should throw if Proton is not installed on Linux', () => {
            ...
            expect(() => ark_command_utils_1.ArkCommandUtils.prepareArkServerCommand('/ark/ark.exe', ['-foo'])).toThrow('Proton is required to run ARK server on Linux but is not installed');
        });
        it('should prepare command for Linux with Proton', () => {
            getPlatform.mockReturnValue('linux');
            isProtonInstalled.mockReturnValue(true);
            const result = ark_command_utils_1.ArkCommandUtils.prepareArkServerCommand('/ark/ark.exe', ['-foo', '-bar'], { TEST: '2' });
