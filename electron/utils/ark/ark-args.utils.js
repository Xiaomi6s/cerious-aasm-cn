...
exports.buildArkServerArgs = buildArkServerArgs;
exports.getArkMapName = getArkMapName;
exports.getArkLaunchParameters = getArkLaunchParameters;
// --- Imports ---
...
function buildArkServerArgs(config) {
    ...
    if (config.sessionName)
        paramParts.push(`SessionName=${config.sessionName}`);
    if (config.gamePort)
        paramParts.push(`Port=${config.gamePort}`);
    if (config.altSaveDirName)
        paramParts.push(`AltSaveDirectoryName=${config.altSaveDirName}`);
    ...
    if (config.queryPort)
        paramParts.push(`QueryPort=${config.queryPort}`);
    ...
    if (peerPort)
        paramParts.push(`PeerPort=${peerPort}`);
    ...
    if (config.serverPassword)
        paramParts.push(`ServerPassword=${config.serverPassword}`);
    // MaxPlayers — must be on the command line; INI alone is not sufficient
    if (config.maxPlayers)
        paramParts.push(`MaxPlayers=${config.maxPlayers}`);
    // PvE mode — command-line args override INI at startup, so both flags must be here
    if (isTrue(config.serverPVE) || isTrue(config.bPvE))
        paramParts.push('ServerPVE=True');
    if (isTrue(config.allowFlyerCarryPvE))
        paramParts.push('AllowFlyerCarryPvE=true');
    ...
    args.push(mainArg);
    // Add standard flags
    if (isFalse(config.battleEye))
        args.push('-NoBattlEye');
    if (isTrue(config.useExclusiveList))
    ...
    if ((0, platform_utils_1.getPlatform)() === 'linux' && !isTrue(config.disableWineCompatFlags)) {
    ...
    if (config.serverPlatform) {
        ...
        const platforms = config.crossplay.map((p) => platformMap[p] || p).join('+');
    ...
    if (isTrue(config.noTransferFromFiltering))
        args.push('-NoTransferFromFiltering');
    if (isTrue(config.preventDownloadSurvivors))
        args.push('-PreventDownloadSurvivors');
    if (isTrue(config.preventDownloadItems))
        args.push('-PreventDownloadItems');
    if (isTrue(config.preventDownloadDinos))
        args.push('-PreventDownloadDinos');
    if (isTrue(config.preventUploadSurvivors))
        args.push('-PreventUploadSurvivors');
    if (isTrue(config.preventUploadItems))
        args.push('-PreventUploadItems');
    if (isTrue(config.preventUploadDinos))
        args.push('-PreventUploadDinos');
    return args;
}
/**
 ...
 */
function getArkLaunchParameters(config) {
    const params = [];
    ...
    if (config.enabledMods && Array.isArray(config.enabledMods)) {
        ...
        if (firstMod && typeof firstMod === 'object' && firstMod.id) {
            ...
            enabledModIds = config.mods.filter((modId) => typeof modId === 'string' && modId.trim());
        }
    }
    ...
    if (config.launchParameters) {
        const additionalParams = config.launchParameters.split(' ').filter((param) => param.trim() !== '');
