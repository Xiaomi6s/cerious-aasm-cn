...
exports.snapshotLogFiles = snapshotLogFiles;
exports.detectAndRegisterLogFile = detectAndRegisterLogFile;
exports.getRegisteredLogFile = getRegisteredLogFile;
exports.unregisterLogFile = unregisterLogFile;
exports.getInstanceLogs = getInstanceLogs;
exports.startArkLogTailing = startArkLogTailing;
exports.cleanupOldLogFiles = cleanupOldLogFiles;
exports.setupLogTailing = setupLogTailing;
...
const instanceSnapshotMap = {};
/**
 * Get the shared ARK server Logs directory path.
 */
function getLogsDir() {
    return path.join((0, ark_server_install_utils_1.getArkServerDir)(), 'ShooterGame', 'Saved', 'Logs');
}
/**
 ...
 */
function snapshotLogFiles() {
    const logsDir = getLogsDir();
    const files = listLogFiles(logsDir);
    const snapshot = new Map();
    for (const f of files)
        snapshot.set(f.path, f.mtime);
    return snapshot;
}
/**
 ...
 */
function detectAndRegisterLogFile(instanceId, sessionName, preStartSnapshot, maxAttempts = 30) {
    ...
    function tryDetect() {
        ...
        const claimedPaths = new Set(Object.entries(instanceLogFileMap)
 ...
 */
function getInstanceLogs(instanceId, maxLines = 200) {
    const state = (0, ark_server_state_utils_1.getInstanceState)(instanceId)?.toLowerCase();
    ...
    let foundLogFile = getRegisteredLogFile(instanceId);
    // Priority 2: Search by session name (only if not registered)
    if (!foundLogFile) {
        let config = {};
        try {
            const baseDir = (0, instance_utils_1.getDefaultInstancesBaseDir)?.() || (0, instance_utils_1.getInstancesBaseDir)?.();
            if (baseDir) {
 ...
 */
function startArkLogTailing(instanceDir, onLog, forceLogFile) {
    // Correct log directory: <arkInstall>/ShooterGame/Saved/Logs
    const logsDir = path.join((0, ark_server_install_utils_1.getArkServerDir)(), 'ShooterGame', 'Saved', 'Logs');
    let logFile = null;
    ...
    let closed = false;
    function findLogFileForSession() {
        if (forceLogFile && fs.existsSync(forceLogFile))
            return forceLogFile;
        if (!fs.existsSync(logsDir))
            return null;
    ...
    function tryAttachWatcher() {
        ...
        if (!fs.existsSync(logsDir)) {
            if (attempts < maxAttempts)
                setTimeout(tryAttachWatcher, 1000);
            return;
        }
 ...
 */
function cleanupOldLogFiles(config, onLog) {
    try {
        const logsDir = path.join((0, ark_server_install_utils_1.getArkServerDir)(), 'ShooterGame', 'Saved', 'Logs');
        if (!fs.existsSync(logsDir))
            return;
        ...
        for (const file of logFiles) {
            ...
            try {
                ...
                if (content.includes(`SessionName=${sessionName}`) ||
                    ...
                    if (onLog)
                        onLog(`[INFO] Cleaned up old log file: ${file}`);
                }
            }
            catch { }
        }
 ...
 */
function setupLogTailing(instanceId, instanceDir, config, onLog, onState) {
    let logTail = null;
    ...
    const wrappedOnLog = (line) => {
        if (!hasAdvertised) {
            const isStartupLine = startupIndicators.some(indicator => line.includes(indicator));
            if (isStartupLine) {
                hasAdvertised = true;
                (0, ark_server_state_utils_1.setInstanceState)(instanceId, 'running');
                if (onState)
                    onState('running');
            }
        }
        if (line.includes('Closing by request') || line.includes('Server shutting down')) {
            (0, ark_server_state_utils_1.setInstanceState)(instanceId, 'stopping');
            if (onState)
                onState('stopping');
        }
        if (onLog)
    ...
    function trySetupTailing() {
        ...
        if (foundLogFile) {
            if (onLog)
                onLog(`[INFO] Tailing log file: ${path.basename(foundLogFile)}`);
