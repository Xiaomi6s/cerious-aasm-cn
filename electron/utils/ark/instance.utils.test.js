...
describe('instance.utils', () => {
    beforeEach(() => {
        ...
        // Setup default mocks
        getDefaultInstallDir.mockReturnValue(mockInstallDir);
    ...
    describe('validateInstanceId (indirect)', () => {
        it('should throw for invalid instance IDs in getInstance', () => {
            // Invalid characters
            expect(() => (0, instance_utils_1.getInstance)('../bad-id')).toThrow('Invalid instance ID format: ../bad-id');
            // Too long
            const longId = 'a'.repeat(51);
            expect(() => (0, instance_utils_1.getInstance)(longId)).toThrow(`Invalid instance ID format: ${longId}`);
        });
        it('should reject invalid instance IDs in saveInstance', async () => {
            const badInstance = { id: '../bad-id', name: 'Bad' };
            await expect((0, instance_utils_1.saveInstance)(badInstance)).rejects.toThrow();
            const longInstance = { id: 'a'.repeat(51), name: 'Long' };
            await expect((0, instance_utils_1.saveInstance)(longInstance)).rejects.toThrow();
        });
    });
    describe('getInstancesBaseDir', () => {
        it('should return the correct instances base directory', () => {
            const result = (0, instance_utils_1.getInstancesBaseDir)();
        ...
        it('should throw error when install directory is not available', () => {
            getDefaultInstallDir.mockReturnValue(null);
            expect(() => (0, instance_utils_1.getInstancesBaseDir)()).toThrow('Could not determine install directory');
        });
    });
    describe('getDefaultInstancesBaseDir', () => {
        it('should be an alias for getInstancesBaseDir', () => {
            const result = (0, instance_utils_1.getDefaultInstancesBaseDir)();
            expect(result).toBe(mockInstancesBaseDir);
        });
    });
    describe('getAllInstances', () => {
        it('should return all valid instances from the directory', async () => {
            ...
            mockedFs.existsSync.mockImplementation((path) => {
                if (path === mockInstancesBaseDir)
                    return false; // Directory doesn't exist initially
                if (path === mockConfigPath1)
                    return true;
                if (path === mockConfigPath2)
                    return true;
                if (path === mockConfigPath3)
                    return false;
                return false;
            });
            mockedFs.readdirSync.mockReturnValue(mockDirs);
            mockedFs.readFileSync
                .mockReturnValueOnce(JSON.stringify(mockInstanceConfig))
                .mockReturnValueOnce(JSON.stringify(mockInstanceConfig2));
            const result = await (0, instance_utils_1.getAllInstances)();
    ...
    describe('getInstance', () => {
        it('should return instance config when it exists', () => {
            ...
            const result = (0, instance_utils_1.getInstance)('test-instance-1');
        ...
        it('should return null when instance does not exist', () => {
            ...
            mockedFs.existsSync.mockReturnValue(false);
            const result = (0, instance_utils_1.getInstance)('nonexistent');
            expect(result).toBe(null);
        });
        it('should return null for invalid instance ID', () => {
            const result = (0, instance_utils_1.getInstance)('');
            expect(result).toBe(null);
        });
        it('should return null for null/undefined instance ID', () => {
            const result1 = (0, instance_utils_1.getInstance)(null);
            const result2 = (0, instance_utils_1.getInstance)(undefined);
    ...
    describe('saveInstance', () => {
        it('should save new instance with generated UUID', async () => {
            ...
            mockedFs.existsSync.mockReturnValue(false);
            mockedFs.readdirSync.mockReturnValue([]);
            const result = await (0, instance_utils_1.saveInstance)(instanceData);
            expect(mockedFs.mkdirSync).toHaveBeenCalledWith(mockDir, { recursive: true });
            expect(mockedFs.writeFileSync).toHaveBeenCalledWith(mockConfigPath, JSON.stringify({ ...instanceData, id: expectedId }, null, 2));
        ...
        it('should save instance with provided ID', async () => {
            ...
            const result = await (0, instance_utils_1.saveInstance)(instanceData);
            expect(mockedFs.mkdirSync).toHaveBeenCalledWith(mockDir, { recursive: true });
            expect(mockedFs.writeFileSync).toHaveBeenCalledWith(mockConfigPath, JSON.stringify(instanceData, null, 2));
        ...
        it('should return error for duplicate server name', async () => {
            ...
            mockedFs.existsSync.mockImplementation((path) => {
                if (path === mockInstancesBaseDir)
                    return true;
                if (path === '/mock/install/dir/AASMServer/ShooterGame/Saved/Servers/existing-1/config.json')
                    return true;
                return false;
            });
            mockedFs.readdirSync.mockReturnValue(['existing-1']);
    ...
    describe('deleteInstance', () => {
        it('should delete existing instance directory', () => {
            ...
            mockedFs.existsSync.mockReturnValue(true);
            const result = (0, instance_utils_1.deleteInstance)(instanceId);
    ...
    describe('error handling', () => {
        it('should handle filesystem errors gracefully', async () => {
            ...
            try {
                await (0, instance_utils_1.getAllInstances)();
                fail('Expected getAllInstances to throw');
            }
            catch (error) {
                expect(error.message).toBe('Filesystem error');
            }
        });
        it('should handle JSON parse errors in getInstance', () => {
            ...
            expect(() => (0, instance_utils_1.getInstance)('test-instance-1')).toThrow();
        });
    });
});
