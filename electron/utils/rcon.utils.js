...
exports.rconClients = void 0;
exports.connectRcon = connectRcon;
exports.disconnectRcon = disconnectRcon;
exports.sendRconCommand = sendRconCommand;
exports.isRconConnected = isRconConnected;
exports.isRconConnecting = isRconConnecting;
exports.cleanupAllRconConnections = cleanupAllRconConnections;
...
const rconConnecting = new Set();
/**
 * Connect to RCON for a given instance.
 */
function connectRcon(instanceId, config, onStatus) {
    if (exports.rconClients[instanceId]) {
        if (onStatus)
            onStatus(true);
        return;
    }
    ...
    function tryConnect() {
        ...
        rcon.on('auth', () => {
            ...
            exports.rconClients[instanceId] = rcon; // 先注册客户端
            rconConnecting.delete(instanceId); // 再清除 connecting 标志
            if (onStatus)
                onStatus(true);
            // 连接建立后意外断开时，清理并通知
            rcon.once('end', () => {
                delete exports.rconClients[instanceId];
                if (onStatus)
                    onStatus(false);
            });
        });
 ...
 */
function disconnectRcon(instanceId) {
    rconConnecting.delete(instanceId);
    if (exports.rconClients[instanceId]) {
        try {
            exports.rconClients[instanceId].disconnect();
        }
        catch (error) {
 ...
 */
function sendRconCommand(instanceId, command) {
    return new Promise((resolve, reject) => {
        const rcon = exports.rconClients[instanceId];
        if (!rcon)
 ...
 */
function isRconConnected(instanceId) {
    return !!exports.rconClients[instanceId];
}
/**
 * Check if a RCON connection attempt is currently in progress for a given instance.
 */
function isRconConnecting(instanceId) {
 ...
 */
function cleanupAllRconConnections() {
    Object.keys(exports.rconClients).forEach((instanceId) => {
