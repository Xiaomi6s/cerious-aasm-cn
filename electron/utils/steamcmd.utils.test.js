...
jest.mock('node-pty', () => ({
    spawn: jest.fn(() => ({
        onData: jest.fn(),
        onExit: jest.fn((cb) => { cb({ exitCode: 0 }); }),
        kill: jest.fn(),
    })),
}));
...
describe('steamcmd.utils', () => {
    ...
    describe('installSteamCmd', () => {
        ...
        it('should install steamcmd on Windows successfully', () => {
            Object.defineProperty(process, 'platform', { writable: true, value: 'win32' });
            mockedFs.existsSync.mockReturnValue(false);
            runInstaller.mockImplementation((config, progressCallback, completionCallback) => {
            ...
            const callArgs = runInstaller.mock.calls[0];
        ...
        it('should initialize SteamCMD after extraction and retry on non-zero exit', () => {
            ...
            expect(callback).toHaveBeenCalledWith(null, 'Installation completed');
            expect(onData).toHaveBeenCalledWith(expect.objectContaining({ step: 'init', message: expect.stringContaining('Initializing') }));
        });
    ...
    describe('progress parsing', () => {
        ...
        it('should cover parseProgress return null case', () => {
            ...
            const parseProgress = (data, lastPercent, estimatedTotal) => {
