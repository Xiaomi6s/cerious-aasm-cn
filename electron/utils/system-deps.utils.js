...
async function checkDependency(dependency) {
    return new Promise((resolve) => {
        ...
        const proc = (0, child_process_1.spawn)('bash', ['-c', dependency.checkCommand], {
...
async function checkAllDependencies() {
    const results = [];
    for (const dependency of exports.LINUX_DEPENDENCIES) {
 ...
 */
function getPackageNameForDistribution(dependency) {
 ...
 */
function generateInstallInstructions(missingDeps) {
    const pkgInfo = getPackageManagerInfo();
    if (!pkgInfo) {
 ...
 */
async function installMissingDependencies(missingDeps, sudoPassword, onProgress) {
    if ((0, platform_utils_1.getPlatform)() !== 'linux') {
    ...
    try {
        ...
        for (const dep of missingDeps) {
            ...
            const aptCandidates = pkgInfo.manager === 'apt' && dep.aptAlternatives?.length
                ? dep.aptAlternatives
                : [packageName];
            onProgress({
                step: 'install',
            ...
            for (const candidate of aptCandidates) {
                try {
                    const extra = pkgInfo.installExtra ? `${pkgInfo.installExtra} ` : '';
 ...
 */
async function runSudoCommand(command, password) {
    return new Promise((resolve, reject) => {
        const proc = (0, child_process_1.spawn)('sudo', ['-S', 'bash', '-c', command], {
 ...
 */
async function validateSudoPassword(password) {
    try {
