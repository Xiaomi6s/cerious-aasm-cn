    ...
    describe('BackupFilenameUtils', () => {
        describe('generateFilename', () => {
            it('should generate manual backup filename with custom name', () => {
                const result = backup_utils_1.BackupFilenameUtils.generateFilename('manual', 'My Custom Backup');
                expect(result).toBe('My Custom Backup.zip');
            });
            it('should sanitize custom name for manual backups', () => {
                const result = backup_utils_1.BackupFilenameUtils.generateFilename('manual', 'My<>:|?*Backup');
            ...
            it('should generate scheduled backup filename with timestamp', () => {
                ...
                const result = backup_utils_1.BackupFilenameUtils.generateFilename('scheduled', 'Daily Backup');
            ...
            it('should generate scheduled backup filename without custom name', () => {
                const mockDate = new Date('2025-09-27T10:30:45.000Z');
                jest.spyOn(global, 'Date').mockImplementation(() => mockDate);
                const result = backup_utils_1.BackupFilenameUtils.generateFilename('scheduled');
            ...
            it('should handle manual backup without custom name', () => {
                ...
                const mockDate = new Date('2025-09-27T10:30:45.000Z');
                jest.spyOn(global, 'Date').mockImplementation(() => mockDate);
                const result = backup_utils_1.BackupFilenameUtils.generateFilename('manual');
        ...
        describe('parseFilename', () => {
            ...
            it('should parse simple manual backup filename', () => {
                const result = backup_utils_1.BackupFilenameUtils.parseFilename('MyBackup.zip', mockFilePath, instanceId);
            ...
            it('should parse structured manual backup filename', () => {
                const result = backup_utils_1.BackupFilenameUtils.parseFilename('manual_20250927103045_MyBackup.zip', mockFilePath, instanceId);
            ...
            it('should parse scheduled backup filename', () => {
                const result = backup_utils_1.BackupFilenameUtils.parseFilename('scheduled_20250927103045_DailyBackup.zip', mockFilePath, instanceId);
                expect(result).toEqual({
            ...
            it('should handle custom names with underscores', () => {
                const result = backup_utils_1.BackupFilenameUtils.parseFilename('scheduled_20250927103045_My_Custom_Backup.zip', mockFilePath, instanceId);
                expect(result).toEqual({
            ...
            it('should return null for invalid structured filename with too few parts', () => {
                ...
                const result = backup_utils_1.BackupFilenameUtils.parseFilename('manual_20250927.zip', mockFilePath, instanceId);
            ...
            it('should return null for invalid timestamp format', () => {
                const consoleSpy = jest.spyOn(console, 'warn').mockImplementation(() => { });
                const result = backup_utils_1.BackupFilenameUtils.parseFilename('scheduled_invalid_MyBackup.zip', mockFilePath, instanceId);
                expect(result).toBeNull();
        ...
        describe('isBackupFile', () => {
            it('should return true for valid zip files', () => {
                expect(backup_utils_1.BackupFilenameUtils.isBackupFile('MyBackup.zip')).toBe(true);
                expect(backup_utils_1.BackupFilenameUtils.isBackupFile('mybackup.ZIP')).toBe(true);
            });
            it('should return true for structured backup files', () => {
                expect(backup_utils_1.BackupFilenameUtils.isBackupFile('manual_20250927103045_Backup.zip')).toBe(true);
                expect(backup_utils_1.BackupFilenameUtils.isBackupFile('scheduled_20250927103045_Backup.zip')).toBe(true);
            });
            it('should return false for non-zip files', () => {
                expect(backup_utils_1.BackupFilenameUtils.isBackupFile('MyBackup.txt')).toBe(false);
                expect(backup_utils_1.BackupFilenameUtils.isBackupFile('MyBackup')).toBe(false);
            });
            it('should return false for system files', () => {
                expect(backup_utils_1.BackupFilenameUtils.isBackupFile('Thumbs.db.zip')).toBe(false);
                expect(backup_utils_1.BackupFilenameUtils.isBackupFile('desktop.ini.zip')).toBe(false);
                expect(backup_utils_1.BackupFilenameUtils.isBackupFile('.DS_Store.zip')).toBe(false);
            });
            it('should return false for hidden files', () => {
                expect(backup_utils_1.BackupFilenameUtils.isBackupFile('.hidden.zip')).toBe(false);
            });
            it('should handle malformed structured filenames', () => {
                ...
                expect(backup_utils_1.BackupFilenameUtils.isBackupFile('manual_invalid_Backup.zip')).toBe(true);
                expect(backup_utils_1.BackupFilenameUtils.isBackupFile('scheduled_invalid_Backup.zip')).toBe(true);
            });
        });
    });
    describe('BackupPathUtils', () => {
        ...
        describe('getInstanceBackupDir', () => {
            it('should return the backup directory path', () => {
                const result = backup_utils_1.BackupPathUtils.getInstanceBackupDir(serverPath);
        ...
        describe('getSettingsFilePath', () => {
            it('should return the backup settings file path', () => {
                const result = backup_utils_1.BackupPathUtils.getSettingsFilePath(serverPath);
        ...
        describe('getBackupFilePath', () => {
            it('should return the full backup file path', () => {
                ...
                const result = backup_utils_1.BackupPathUtils.getBackupFilePath(serverPath, filename);
                expect(mockPath.join).toHaveBeenCalledWith(serverPath, 'backups');
