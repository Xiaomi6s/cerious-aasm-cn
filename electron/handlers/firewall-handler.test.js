...
const mockFirewallService = firewall_service_1.firewallService;
// Import the handler to register the event listeners
require("../handlers/firewall-handler");
// Get the registered event handlers
const setupArkServerFirewallHandler = mockMessagingService.on.mock.calls.find(call => call[0] === 'setup-ark-server-firewall')?.[1];
const setupWebServerFirewallHandler = mockMessagingService.on.mock.calls.find(call => call[0] === 'setup-web-server-firewall')?.[1];
const getLinuxFirewallInstructionsHandler = mockMessagingService.on.mock.calls.find(call => call[0] === 'get-linux-firewall-instructions')?.[1];
const checkFirewallEnabledHandler = mockMessagingService.on.mock.calls.find(call => call[0] === 'check-firewall-enabled')?.[1];
const mockSender = {};
    ...
    describe('check-firewall-enabled event', () => {
        ...
        it('should handle exception and return error branch', async () => {
            ...
            jest.spyOn(platformUtils, 'getPlatform').mockImplementation(() => {
                callCount++;
                if (callCount === 1)
                    throw error; // throw on first call (try block)
