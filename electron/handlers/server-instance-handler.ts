 ...
 */
messagingService.on('stop-server-instance', async (payload, sender) => {
  ...
  try {
    ...
    messagingService.sendToOriginator('stop-server-instance', {
      ...result,
      requestId
    }, sender);
  } catch (error) {
    ...
    messagingService.sendToOriginator('stop-server-instance', {
      success: false,
      error: (error as Error)?.message || 'Failed to stop server',
      requestId
    }, sender);
  }
});
