...
const mockInstallService = install_service_1.installService;
// Import the handler to register the event listeners
require("../handlers/install-handler");
// Get the registered event handlers
const checkInstallRequirementsHandler = mockMessagingService.on.mock.calls.find(call => call[0] === 'check-install-requirements')?.[1];
const installHandler = mockMessagingService.on.mock.calls.find(call => call[0] === 'install')?.[1];
const cancelInstallHandler = mockMessagingService.on.mock.calls.find(call => call[0] === 'cancel-install')?.[1];
const mockSender = {};
describe('Install Handler', () => {
    ...
    describe('check-install-requirements event', () => {
        ...
        it('should handle check install requirements with undefined payload (cover || {} branch)', async () => {
            ...
            expect(mockInstallService.checkInstallRequirements).toHaveBeenCalledWith(undefined);
            expect(mockMessagingService.sendToOriginator).toHaveBeenCalledWith('check-install-requirements', {
                ...expectedResult,
                requestId: undefined
            }, mockSender);
    ...
    describe('install event', () => {
        it('should send progress data as-is when requestId is missing (cover fallback branch)', async () => {
            ...
            const calls = mockMessagingService.sendToOriginator.mock.calls.filter(call => call[0] === 'install' && call[1]?.data === 'No ID');
        ...
        it('should handle valid progress data (cover parsed.step && parsed.percent !== undefined branch)', async () => {
            ...
            const found = calls.some(call => call[0] === 'install' &&
        ...
        it('should handle install with undefined payload (cover || {} branch)', async () => {
            ...
            const callArgs = mockInstallService.installComponent.mock.calls[0];
    ...
    describe('cancel-install event', () => {
        ...
        it('should handle cancel-install with undefined payload (cover || {} branch)', () => {
            ...
            expect(mockInstallService.cancelInstallation).toHaveBeenCalledWith(undefined);
            expect(mockMessagingService.sendToOriginator).toHaveBeenCalledWith('cancel-install', {
                target: 'default',
