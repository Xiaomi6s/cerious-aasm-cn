...
const linux_deps_service_1 = require("../services/linux-deps.service");
// Mock the services
jest.mock('../services/messaging.service');
jest.mock('../services/linux-deps.service');
const mockMessagingService = messaging_service_1.messagingService;
const mockLinuxDepsService = jest.mocked(linux_deps_service_1.LinuxDepsService.prototype);
// Import the handler to register the event listeners
require("../handlers/linux-deps-handler");
// Get the registered event handlers
const checkLinuxDepsHandler = mockMessagingService.on.mock.calls.find(call => call[0] === 'check-linux-deps')?.[1];
const validateSudoPasswordHandler = mockMessagingService.on.mock.calls.find(call => call[0] === 'validate-sudo-password')?.[1];
const installLinuxDepsHandler = mockMessagingService.on.mock.calls.find(call => call[0] === 'install-linux-deps')?.[1];
const getLinuxDepsListHandler = mockMessagingService.on.mock.calls.find(call => call[0] === 'get-linux-deps-list')?.[1];
const mockSender = {
    send: jest.fn()
};
