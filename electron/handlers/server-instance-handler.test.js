...
const arkServerStateUtils = __importStar(require("../utils/ark/ark-server/ark-server-state.utils"));
const mockMessagingService = messaging_service_1.messagingService;
const mockServerInstanceService = server_instance_service_1.serverInstanceService;
const mockServerProcessService = server_process_service_1.serverProcessService;
const mockServerMonitoringService = server_monitoring_service_1.serverMonitoringService;
const mockServerOperationsService = server_operations_service_1.serverOperationsService;
const mockServerManagementService = server_management_service_1.serverManagementService;
const mockAutomationService = automation_service_1.automationService;
const mockGetNormalizedInstanceState = arkServerStateUtils.getNormalizedInstanceState;
// Store handler functions for testing
let forceStopHandler;
let getStateHandler;
let getLogsHandler;
let connectRconHandler;
let disconnectRconHandler;
let getRconStatusHandler;
let rconCommandHandler;
let startInstanceHandler;
let getPlayersHandler;
let getInstancesHandler;
let getInstanceHandler;
let saveInstanceHandler;
let deleteInstanceHandler;
let importBackupHandler;
...
describe('Server Instance Handler', () => {
    describe('start-server-instance handler', () => {
        it('should not send notification if result.started is false and result.portError is set but sender has no send function', async () => {
            ...
            const notifCall = mockMessagingService.sendToAll.mock.calls.find((c) => c[0] === 'notification');
...
describe('save-server-instance handler', () => {
    ...
    it('should send notification with Unknown fallback if name and id are missing', async () => {
        ...
        const call = mockMessagingService.sendToAllOthers.mock.calls.find((c) => c[0] === 'notification');
    ...
    it('should not send notification if saveInstance result.success is false', async () => {
        ...
        const notifCall = mockMessagingService.sendToAllOthers.mock.calls.find((c) => c[0] === 'notification');
        expect(notifCall).toBeUndefined();
        // Should send result to originator
        const call = mockMessagingService.sendToOriginator.mock.calls.find((c) => c[0] === 'save-server-instance');
        expect(call).toBeDefined();
...
describe('delete-server-instance handler', () => {
    ...
    it('should not send notification if result.success is false', async () => {
        ...
        const notifCall = mockMessagingService.sendToAllOthers.mock.calls.find((c) => c[0] === 'notification');
        expect(notifCall).toBeUndefined();
        const call = mockMessagingService.sendToOriginator.mock.calls.find((c) => c[0] === 'delete-server-instance');
        expect(call).toBeDefined();
        expect(call[1].success).toBe(false);
...
describe('payload fallback coverage', () => {
    ...
    it('start-server-instance: should use fallback error message if thrown value is string', async () => {
        ...
        const call = mockMessagingService.sendToOriginator.mock.calls.find((c) => c[0] === 'start-server-instance');
        expect(call).toBeDefined();
        expect(call[1].success).toBe(false);
    ...
    it('connect-rcon: should use fallback error message if thrown value is string', async () => {
        ...
        const call = mockMessagingService.sendToOriginator.mock.calls.find((c) => c[0] === 'connect-rcon');
        expect(call).toBeDefined();
        expect(call[1].success).toBe(false);
    ...
    it('rcon-command: should use fallback error message if thrown value is string', async () => {
        ...
        const call = mockMessagingService.sendToOriginator.mock.calls.find((c) => c[0] === 'rcon-command');
        expect(call).toBeDefined();
    ...
    it('import-server-from-backup: should use fallback error message if thrown value is string', async () => {
        ...
        const call = mockMessagingService.sendToOriginator.mock.calls.find((c) => c[0] === 'import-server-from-backup');
        expect(call).toBeDefined();
        expect(call[1].success).toBe(false);
    ...
    it('should call sendToAll in startPlayerPolling callback (connect-rcon)', async () => {
        ...
        const call = mockMessagingService.sendToAll.mock.calls.find((c) => c[0] === 'server-instance-players');
        expect(call).toBeDefined();
    ...
    it('should send portError notification if present and sender.send is function (start-server-instance)', async () => {
        ...
        const call = senderWithSend.send.mock.calls.slice(-1)[0];
    ...
    it('get-rcon-status: should handle undefined payload', async () => {
        mockServerOperationsService.getRconStatus.mockResolvedValue({ success: true, connected: true, instanceId: '' });
    ...
    it('start-server-instance: should handle undefined payload', async () => {
        ...
        const call = mockServerInstanceService.startServerInstance.mock.calls.slice(-1)[0];
    ...
    it('get-server-instance-players: should handle undefined payload', async () => {
        ...
        const call = mockServerMonitoringService.getPlayerCount.mock.calls.slice(-1)[0];
        expect(call[0]).toBeUndefined();
    });
    ...
    it('get-server-instance: should handle undefined payload', async () => {
        ...
        const call = mockServerManagementService.getInstance.mock.calls.slice(-1)[0];
        expect(call[0]).toBeUndefined();
    });
    ...
    it('delete-server-instance: should handle undefined payload', async () => {
        ...
        const call = mockServerManagementService.deleteInstance.mock.calls.slice(-1)[0];
        expect(call[0]).toBeUndefined();
    });
    it('import-server-from-backup: should handle undefined payload', async () => {
        ...
        const call = mockServerInstanceService.importServerFromBackup.mock.calls.slice(-1)[0];
        expect(call[0]).toBeUndefined();
...
describe('force-stop-server-instance handler', () => {
    it('should send fallback error message if an error is thrown', async () => {
        ...
        const call = mockMessagingService.sendToOriginator.mock.calls.find((c) => c[0] === 'force-stop-server-instance');
        expect(call).toBeDefined();
        expect(call[1].success).toBe(false);
...
describe('force-stop-server-instance handler', () => {
    it('should not send notification if result.success is false', async () => {
        ...
        expect(notifCall).toBeUndefined();
        // Should send result to originator
        const originCall = mockMessagingService.sendToOriginator.mock.calls.find((c) => c[0] === 'force-stop-server-instance');
