...
describe('ServerManagementService', () => {
    ...
    let validateServerNameMock;
    let validatePortMock;
    let instanceUtilsMock;
    let backupServiceMock;
    let generateRandomPasswordMock;
    let serverProcessServiceMock;
    let serverMonitoringServiceMock;
    let serverLifecycleServiceMock;
    let getNormalizedInstanceStateMock;
    ...
    describe('getInstance', () => {
        it('should get instance by id', async () => {
            ...
            const result = await server_management_service_1.serverManagementService.getInstance('instance1');
            expect(result).toEqual({ instance });
        });
        it('should return null for empty id', async () => {
            const result = await server_management_service_1.serverManagementService.getInstance('');
    ...
    describe('saveInstance', () => {
        ...
        it('should handle invalid instance object', async () => {
            const result = await server_management_service_1.serverManagementService.saveInstance(null);
            expect(result).toEqual({
                success: false,
    ...
    describe('deleteInstance', () => {
        it('should delete instance successfully', async () => {
            ...
            const result = await server_management_service_1.serverManagementService.deleteInstance(instanceId);
            expect(result).toEqual({
                success: true,
                id: instanceId
            });
        ...
        it('should handle invalid instance id', async () => {
            validateInstanceIdMock.mockReturnValue(false);
            const result = await server_management_service_1.serverManagementService.deleteInstance('invalid');
            expect(result).toEqual({
                success: false,
                id: 'invalid'
            });
        ...
        it('should handle instance not found', async () => {
            ...
            const result = await server_management_service_1.serverManagementService.deleteInstance('instance1');
            expect(result).toEqual({
                success: false,
                id: 'instance1'
            });
    ...
    describe('createInstance', () => {
        it('should create instance successfully', async () => {
            ...
            validatePortMock.mockReturnValue(true);
            instanceUtilsMock.saveInstance.mockResolvedValue(savedInstance);
            const result = await server_management_service_1.serverManagementService.createInstance(instanceData);
    ...
    describe('importFromBackup', () => {
        it('should import from backup successfully', async () => {
            ...
            const result = await server_management_service_1.serverManagementService.importFromBackup(backupPath, instanceName);
            expect(result).toEqual({
                success: true,
        ...
        it('should handle invalid backup path', async () => {
            const result = await server_management_service_1.serverManagementService.importFromBackup('', 'Server Name');
            expect(result).toEqual({
                success: false,
        ...
        it('should handle invalid instance name', async () => {
            const result = await server_management_service_1.serverManagementService.importFromBackup('/path/backup.zip', '');
            expect(result).toEqual({
                success: false,
        ...
        it('should handle exception', async () => {
            ...
            const result = await server_management_service_1.serverManagementService.importFromBackup('/path/backup.zip', 'Server Name');
            expect(result).toEqual({
                success: false,
                error: 'Import error'
            });
    ...
    describe('prepareInstanceConfiguration', () => {
        it('should generate RCON password when missing', async () => {
            ...
            await server_management_service_1.serverManagementService.prepareInstanceConfiguration(instanceId, instance);
            expect(generateRandomPasswordMock).toHaveBeenCalledWith(16);
            expect(instance.rconPassword).toBe('generated_password');
            expect(fsMock.writeFileSync).toHaveBeenCalledWith('/instances/instance1/config.json', JSON.stringify(instance, null, 2), 'utf8');
        });
    ...
    describe('cloneInstance', () => {
        it('should clone instance successfully', async () => {
            ...
            const result = await server_management_service_1.serverManagementService.cloneInstance(sourceInstanceId, newInstanceName);
            expect(result.success).toBe(true);
        ...
        it('should handle invalid source instance id', async () => {
            validateInstanceIdMock.mockReturnValue(false);
            const result = await server_management_service_1.serverManagementService.cloneInstance('invalid', 'New Name');
            expect(result).toEqual({
                success: false,
        ...
        it('should handle invalid instance name', async () => {
            validateInstanceIdMock.mockReturnValue(true);
            validateServerNameMock.mockReturnValue(false);
            const result = await server_management_service_1.serverManagementService.cloneInstance('source1', 'Invalid@Name');
            expect(result).toEqual({
                success: false,
                error: 'Invalid instance name'
            });
        ...
        it('should handle source instance not found', async () => {
            ...
            validateServerNameMock.mockReturnValue(true);
            instanceUtilsMock.getInstance.mockResolvedValue(null);
            const result = await server_management_service_1.serverManagementService.cloneInstance('source1', 'New Name');
            expect(result).toEqual({
                success: false,
