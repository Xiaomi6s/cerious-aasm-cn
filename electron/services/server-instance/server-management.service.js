...
exports.serverManagementService = exports.ServerManagementService = void 0;
...
class ServerManagementService {
    ...
    async saveInstance(instance) {
        try {
            ...
            if (instance.id !== undefined && !(0, validation_utils_1.validateInstanceId)(instance.id)) {
                return {
                    success: false,
            ...
            if (instance.name && !(0, validation_utils_1.validateServerName)(instance.name)) {
                return {
                    success: false,
            ...
            if (instance.port !== undefined && !(0, validation_utils_1.validatePort)(instance.port)) {
                return {
                    success: false,
            ...
            if (saved && saved.id && instance.useExclusiveList) {
                try {
                    ...
                    if (instance.exclusiveJoinPlayers && Array.isArray(instance.exclusiveJoinPlayers)) {
                        playerIds = instance.exclusiveJoinPlayers.map((player) => player.playerId).filter((id) => id && id.trim());
                    }
                    else if (instance.exclusiveJoinPlayerIds && Array.isArray(instance.exclusiveJoinPlayerIds)) {
                        // Fallback to old format for backward compatibility
                        playerIds = instance.exclusiveJoinPlayerIds.filter((id) => id && id.trim());
                    }
     ...
     */
    async importFromBackup(backupPath, instanceName) {
        try {
            ...
            const instance = await backup_service_1.backupService.importBackupAsNewServer(instanceName, backupPath);
            return {
                success: true,
                instance: instance
            };
     ...
     */
    async prepareInstanceConfiguration(instanceId, instance) {
        const path = require('path');
        const fs = require('fs');
            ...
            if (!fs.existsSync(sourceDir)) {
                ...
                if (await fsExtra.pathExists(sourceContent)) {
                    ...
                    if (await fsExtra.pathExists(destContent)) {
                        ...
                        if (stat.isSymbolicLink())
                            await fsExtra.unlink(destContent);
                    }
                ...
                if (await fsExtra.pathExists(sourceEngine)) {
                    if (await fsExtra.pathExists(engineDir)) {
                        ...
                        if (stat.isSymbolicLink())
                            await fsExtra.unlink(engineDir);
                    }
                    if (!(await fsExtra.pathExists(engineDir))) {
        ...
        if (!instance.rconPassword) {
            instance.rconPassword = (0, crypto_utils_1.generateRandomPassword)(16);
     ...
     */
    async cloneInstance(sourceInstanceId, newInstanceName) {
        try {
            if (!(0, validation_utils_1.validateInstanceId)(sourceInstanceId)) {
                return {
                    success: false,
                    error: 'Invalid source instance ID'
                };
            }
            if (!(0, validation_utils_1.validateServerName)(newInstanceName)) {
                return {
                    success: false,
                    error: 'Invalid instance name'
                };
...
exports.ServerManagementService = ServerManagementService;
// Export singleton instance
exports.serverManagementService = new ServerManagementService();
