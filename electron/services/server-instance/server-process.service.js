...
exports.serverProcessService = exports.ServerProcessService = void 0;
...
class ServerProcessService {
     ...
     */
    getNormalizedInstanceState(instanceId) {
    ...
    async startServerProcess(instanceId, instance) {
        ...
        const formattedLogDir = path.join(ark_utils_1.ArkPathUtils.getArkServerDir(), 'ShooterGame', 'Saved', 'Logs').replace(/\\/g, '/');
        // Build the ARK server command arguments
        const args = (0, ark_utils_1.buildArkServerArgs)({
            ...instance,
        ...
        const commandInfo = prepareArkServerCommand(ark_utils_1.ArkPathUtils.getArkExecutablePath(), args);
        // Set up spawn options with proper environment and working directory
        const { getPlatform } = require('../../utils/platform.utils');
        const cwd = getPlatform() === 'windows' ? instanceDir : ark_utils_1.ArkPathUtils.getArkServerDir();
        ...
        try {
            ...
            fs.writeFileSync(path.join(cwd, 'steam_appid.txt'), ark_utils_1.ARK_APP_ID, 'utf8');
        }
        catch (e) {
        ...
        const spawnOptions = {
            ...
            env: {
                ...
                SteamAppId: ark_utils_1.ARK_APP_ID,
        ...
        const logSnapshot = (0, ark_server_logging_utils_1.snapshotLogFiles)();
        // Start the server process
        const serverProcess = (0, child_process_1.spawn)(commandInfo.command, commandInfo.args, spawnOptions);
        ...
        (0, ark_server_logging_utils_1.detectAndRegisterLogFile)(instanceId, sessionName, logSnapshot);
        // Notify Discord
     ...
     */
    setupProcessMonitoring(instanceId, instance, onLog, onState) {
        ...
        serverProcess.on('exit', async (code) => {
            ...
            (0, ark_server_logging_utils_1.unregisterLogFile)(instanceId);
            onState?.('stopped');
            // Notify Discord
            const { discordService } = require('../discord.service');
        ...
        const triggerRconConnect = async () => {
            rconTriggered = true;
            // 使用启动时的 instance 配置（而非重新读文件），确保密码与服务器启动时一致
            const rconPort = instance?.rconPort;
            const rconPassword = instance?.serverAdminPassword || instance?.rconPassword;
            if (!rconPort || !rconPassword) {
                console.log(`[server-process-service] RCON not configured for ${instanceId} — skipping connect`);
                return;
            }
            console.log(`[server-process-service] Server ${instanceId} is up — attempting RCON connection (port=${rconPort})`);
            const messagingService = require('../messaging.service').messagingService;
            const { connectRcon } = require('../../utils/rcon.utils');
            // 直接用启动时的配置连接，避免读取可能已被修改的 config.json
            connectRcon(instanceId, instance, (connected) => {
                messagingService.sendToAll('rcon-status', { instanceId, connected });
            });
        };
        const wrappedOnState = (state) => {
            onState?.(state);
     ...
     */
    async stopServerProcess(instanceId) {
...
exports.ServerProcessService = ServerProcessService;
// Export singleton instance
exports.serverProcessService = new ServerProcessService();
