...
describe('BackupCleanupService', () => {
    let service;
    beforeEach(() => {
        service = new backup_cleanup_service_1.BackupCleanupService();
    });
    ...
    describe('cleanupOldBackups', () => {
        it('should do nothing if backups <= maxBackupsToKeep', async () => {
            ...
            const unlink = jest.spyOn(require('fs'), 'unlink').mockImplementation((...args) => args[1](null));
    ...
    describe('cleanupArkSaveFiles', () => {
        ...
        it('should delete oldest .bak files if more than maxBackupsToKeep', async () => {
            ...
            jest.spyOn(service, 'collectBakFiles').mockImplementation(async (...args) => {
                const arr = args[1];
    ...
    describe('collectBakFiles', () => {
        it('should collect .bak files recursively', async () => {
            // Setup mock fs
            const readdir = jest.spyOn(require('fs'), 'readdir').mockImplementation((...args) => {
                const dir = args[0];
                const cb = args[1];
                if (dir.endsWith('root'))
                    cb(null, ['sub', 'file1.bak']);
                else
                    cb(null, ['file2.bak']);
            });
            const stat = jest.spyOn(require('fs'), 'stat').mockImplementation((...args) => {
                const file = args[0];
                const cb = args[1];
                cb(null, {
            ...
            const bakFiles = [];
            await service.collectBakFiles('root', bakFiles);
        ...
        it('should handle errors in collectBakFiles', async () => {
            ...
            await service.collectBakFiles('root', []);
