...
exports.arkConfigService = exports.ArkConfigService = void 0;
...
class ArkConfigService {
    constructor() {
        ...
        this.asaSettingsMapping = [
            ...
            { key: "enableCryoSicknessPVE", iniKey: "EnableCryoSicknessPVE", destination: "GameUserSettings.ini", section: "[ServerSettings]" },
            { key: "enableCryopodNerf", iniKey: "EnableCryopodNerf", destination: "GameUserSettings.ini", section: "[ServerSettings]" },
            { key: "cryopodNerfDuration", iniKey: "CryopodNerfDuration", destination: "GameUserSettings.ini", section: "[ServerSettings]" },
            { key: "cryopodNerfDamageMult", iniKey: "CryopodNerfDamageMult", destination: "GameUserSettings.ini", section: "[ServerSettings]" },
            { key: "cryopodNerfIncomingDamageMultPercent", iniKey: "CryopodNerfIncomingDamageMultPercent", destination: "GameUserSettings.ini", section: "[ServerSettings]" },
            // =====================================================
            ...
            { key: "mapPlayerLocation", iniKey: "ShowMapPlayerLocation", destination: "GameUserSettings.ini", section: "[ServerSettings]" },
     ...
     */
    writeArkConfigFiles(instanceDir, config) {
        try {
     ...
     */
    getArkLaunchParameters(config) {
        // Extract launch parameters from config
        return config.launchParameters ? config.launchParameters.split(' ').filter((param) => param.trim() !== '') : [];
    }
    /**
     ...
     */
    readIniFile(instanceId, filename) {
     ...
     */
    parseIniToConfig(filename, content) {
        // Build reverse lookup: lowercase(iniKey) → camelCase key
        const iniKeyToConfig = new Map();
     ...
     */
    writeIniFile(instanceId, filename, content) {
        const { getInstancesBaseDir } = require('../utils/ark/instance.utils');
        const instanceDir = path.join(getInstancesBaseDir(), instanceId);
        const configDir = path.join(instanceDir, 'Config', 'WindowsServer');
        if (!fs.existsSync(configDir)) {
...
exports.ArkConfigService = ArkConfigService;
// Export singleton instance
exports.arkConfigService = new ArkConfigService();
