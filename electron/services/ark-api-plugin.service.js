...
class ArkApiPluginService {
     ...
     */
    removePlugin(instanceId, folderName) {
     ...
     */
    async getLatestAsaApiRelease() {
        return new Promise((resolve, reject) => {
            const req = https.get(ASAAPI_RELEASES_URL, { headers: { 'User-Agent': 'Cerious-AASM' } }, (res) => {
                let data = '';
     ...
     */
    async downloadAsaApi(instanceId, downloadUrl) {
        const { getInstancesBaseDir } = require('../utils/ark/instance.utils');
        const instanceDir = path.join(getInstancesBaseDir(), instanceId);
     ...
     */
    installPluginFromZipPath(instanceId, zipPath) {
        const pluginDir = this.getPluginDir(instanceId);
        if (!fs.existsSync(pluginDir)) {
     ...
     */
    async installPluginFromUrl(instanceId, url) {
        const pluginDir = this.getPluginDir(instanceId);
        if (!fs.existsSync(pluginDir)) {
            fs.mkdirSync(pluginDir, { recursive: true });
        }
...
exports.ArkApiPluginService = ArkApiPluginService;
exports.arkApiPluginService = new ArkApiPluginService();
