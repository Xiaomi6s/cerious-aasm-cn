...
class ArkUpdateService {
    ...
    async initialize() {
        try {
            this.installedBuildId = await (0, ark_server_utils_1.getCurrentInstalledVersion)();
     ...
     */
    async scheduleClusterUpdate(minutes) {
        ...
        const runningInstances = instances.filter((i) => require('./server-instance/server-process.service').serverProcessService.getNormalizedInstanceState(i.id) === 'running');
        // Warning Loop
     ...
     */
    async performClusterUpdate(preRunningInstances) {
        ...
        if (!preRunningInstances) {
            const instances = (await serverManagementService.getAllInstances()).instances;
            preRunningInstances = instances.filter((i) => serverProcessService.getNormalizedInstanceState(i.id) === 'running');
        }
        ...
        await Promise.all(preRunningInstances.map((inst) => serverLifecycleService.stopServerInstance(inst.id)));
        ...
        try {
            ...
            await installArkServer((progress) => {
                this.messagingService.sendToAll('cluster-update-progress', progress);
            });
            this.installedBuildId = await (0, ark_server_utils_1.getCurrentInstalledVersion)(); // Refresh version
        }
        catch (e) {
    ...
    async getLatestServerVersion() {
        ...
        return new Promise((resolve) => {
            ...
            const proc = (0, child_process_1.spawn)(steamcmdExecutable, [
                '+login', 'anonymous',
                '+app_info_update', '1',
                '+app_info_print', ark_utils_1.ARK_APP_ID.toString(),
                '+quit'
            ], { cwd: steamcmdPath });
...
exports.ArkUpdateService = ArkUpdateService;
