...
describe('MessagingService', () => {
    let service;
    beforeEach(() => {
        service = new messaging_service_1.MessagingService();
    });
    it('setApiProcess sets apiProcess', () => {
        const proc = { send: jest.fn() };
        service.setApiProcess(proc);
        expect(service.apiProcess).toBe(proc);
    });
    ...
    it('addWebContents adds and removes webContents', () => {
        const wc = { on: jest.fn((event, cb) => { if (event === 'destroyed')
                cb(); }) };
    ...
    it('sendToAllWebSockets sends to all open clients except excluded', () => {
        ...
        service.wsServer = { clients: [client1, client2] };
